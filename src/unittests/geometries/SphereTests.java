...
public class SphereTests {
	...
	public void testFindIntersections() {
		...
		List<Point3D> result1 = sphere.findIntersections(new Ray(new Point3D(1, 0.5, 0), new Vector(3, 1, 0)));
		...
		assertNull("Ray's line after the sphere",
				sphere.findIntersections(new Ray(new Point3D(-1, 0, 0), new Vector(-2, -1, 0))));
		...
		List<Point3D> result2 = sphere.findIntersections(new Ray(new Point3D(0.4, 0.5, 0), new Vector(3, 1, 0)));
		...
		List<Point3D> result3 = sphere.findIntersections(new Ray(new Point3D(2.5, 0, 0), new Vector(-1, 0, 0)));
		...
		List<Point3D> result4 = sphere.findIntersections(new Ray(new Point3D(0.4, 0, 0), new Vector(-1, 0, 0)));
		assertEquals("Wrong number of points", 1, result4.size());
		assertEquals("Ray coming out of a sphere", List.of(ZERO), result4);

		// TC15: Ray starts inside (1 points)
		List<Point3D> result5 = sphere.findIntersections(new Ray(new Point3D(2, 0, 0), new Vector(-1, 0, 0)));
		assertEquals("Wrong number of points", 1, result5.size());
		assertEquals("Ray coming out of a sphere", List.of(ZERO), result5);

		// TC16: Ray starts at the center (1 points)
		List<Point3D> result6 = sphere.findIntersections(new Ray(new Point3D(1, 0, 0), new Vector(-1, 0, 0)));
		assertEquals("Wrong number of points", 1, result6.size());
		assertEquals("Ray coming out of a sphere", List.of(ZERO), result6);

		// TC17: Ray starts at sphere and goes outside (0 points)
		assertNull("Ray's line out of sphere",
				sphere.findIntersections(new Ray(new Point3D(0, 0, 1), new Vector(-1, 0, 1))));

		// TC18: Ray starts after sphere (0 points)
		assertNull("Ray's line out of sphere",
				sphere.findIntersections(new Ray(new Point3D(-1, 0, 0), new Vector(-2, 0, 0))));

		// **** Group: Ray's line is tangent to the sphere (all tests 0 points)
		// TC19: Ray starts before the tangent point (0 points)
		assertNull("Ray's line out of sphere",
				sphere.findIntersections(new Ray(new Point3D(0, 0, -1), new Vector(0, 1, 0))));

		// TC20: Ray starts at the tangent point (0 points)
		assertNull("Ray's line out of sphere",
				sphere.findIntersections(new Ray(new Point3D(1, 1, 0), new Vector(3, 2, 1))));
	...
	/**
	 * Test method for
	 * {@link geometries.Sphere#findGeoIntersections(primitives.Point3D)}.
	 */
	@Test
	public void testFindGeoIntersections() {
		Sphere sphere = new Sphere(1d, new Point3D(1, 0, 0));

		// TC01: Ray intersects the sphere (2 point)
		// and the points in the range of the maximum distance
		// ((maxDistance - t1) >= 0 && (maxDistance - t2) >= 0)
		List<GeoPoint> result3 = sphere.findGeoIntersections(new Ray(new Point3D(2.5, 0, 0), 
				new Vector(-1, 0, 0)), 3);
		assertEquals("Wrong number of points", 2, result3.size());

		// TC02: Ray intersects the sphere (1 point)
		// and one point in the range of the maximum distance
        //(maxDistance - t1) > 0)
		List<GeoPoint> result2 = sphere.findGeoIntersections(new Ray(new Point3D(2.5, 0, 0),
				new Vector(-1, 0, 0)), 1);
		assertEquals("Wrong number of points", 1, result2.size());

		// TC03: Ray intersects the sphere 
		// but the point is not in the range of the maximum distance 
        //((maxDistance - t1) < 0 && (maxDistance - t2) < 0)
		assertNull("Ray's line out of sphere",
				sphere.findGeoIntersections(new Ray(new Point3D(1, 1, 0), 
						new Vector(1, 0, 0)), 0.4));
